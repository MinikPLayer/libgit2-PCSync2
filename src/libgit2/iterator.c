...
static void iterator_set_ignore_case(git_iterator *iter, bool ignore_case)
{
	...
	iter->entry_srch = ignore_case ? git_index_entry_isrch :
...
static int
iterator_range_init(git_iterator *iter, const char *start, const char *end)
{
	if (start && *start) {
...
static int
iterator_reset_range(git_iterator *iter, const char *start, const char *end)
{
...
static int iterator_pathlist_init(git_iterator *iter, git_strarray *pathlist)
{
	...
	for (i = 0; i < pathlist->count; i++) {
		...
		if (git_vector_insert(&iter->pathlist, pathlist->strings[i]) <
		    0)
			return -1;
	}
        ...
        git_iterator_options *given_opts)
{
	...
	if (repo && (iter->flags & GIT_ITERATOR_PRECOMPOSE_UNICODE) == 0 &&
		...
		if (git_repository__configmap_lookup(
		            &precompose, repo, GIT_CONFIGMAP_PRECOMPOSE) < 0)
			git_error_clear();
		else if (precompose)
	...
	if ((error = iterator_range_init(iter, options->start, options->end)) <
	            0 ||
...
iterator_has_started(git_iterator *iter, const char *path, bool is_submodule)
{
	 ...
	 */
	if (is_submodule && iter->start_len &&
	    path_len == iter->start_len - 1 &&
...
static bool iterator_pathlist_next_is(git_iterator *iter, const char *path)
{
	...
	for (i = iter->pathlist_walk_idx; i < iter->pathlist.length; i++) {
		 ...
		 */
		if (cmp == 0) {
			/* if this pathlist entry is not suffixed with a '/'
			 * then it matches a path that is a file or a directory.
			 ...
			 * (eg, pathlist = "foo/" and path is "foo/" or
			 * "foo/something")
			 */
...
iterator_pathlist_search(git_iterator *iter, const char *path, size_t path_len)
{
	int (*vector_cmp)(const void *a, const void *b);
	const char *p;
	size_t idx;
	 ...
	 * don't know that yet, since we're avoiding a stat unless it's
	 * necessary) so walk the pathlist looking for the given path with a '/'
	 * after it,
	 */
	while ((p = git_vector_get(&iter->pathlist, idx)) != NULL) {
		...
		/* is this a literal directory entry (eg `foo/`) or a file
		 * beneath */
		if (p[path_len] == '/') {
...
int git_iterator_for_nothing(git_iterator **out, git_iterator_options *options)
{
	...
	static git_iterator_callbacks callbacks = {
		empty_iterator_noop,  empty_iterator_noop,
		empty_iterator_noop,  empty_iterator_advance_over,
...
{
	return iter->frames.size ? &iter->frames.ptr[iter->frames.size - 1] :
	                           NULL;
}
...
static int tree_iterator_compute_path(git_str *out, tree_iterator_entry *entry)
{
	git_str_clear(out);

	if (entry->parent_path)
		git_str_joinpath(
		        out, entry->parent_path, entry->tree_entry->filename);
	else
        ...
        tree_iterator_entry *frame_entry)
{
	...
	if (frame_entry && (error = tree_iterator_compute_path(
	                            &new_frame->path, frame_entry)) < 0)
		goto done;
	...
	git_array_foreach (dup->entries, i, tree_entry) {
		if ((new_entry = git_pool_malloc(&iter->entry_pool, 1)) ==
		    NULL) {
			git_error_set_oom();
			error = -1;
			goto done;
		...
		if ((error = git_vector_insert(
		             &new_frame->entries, new_entry)) < 0)
			goto done;
	}

	git_vector_set_sorted(
	        &new_frame->entries, !iterator__ignore_case(&iter->base));
        ...
        const char *filename)
{
	...
	while (parent_frame->next_idx < parent_frame->entries.length) {
		...
		if ((error = git_tree_lookup(
		             &tree, iter->base.repo, &entry->tree_entry->oid)) <
		    0)
			break;
		...
		GIT_ERROR_CHECK_ALLOC_ADD(
		        &new_size, frame->entries.length, tree->entries.size);
...
tree_iterator_frame_push(tree_iterator *iter, tree_iterator_entry *entry)
{
	...
	if ((error = git_tree_lookup(
	             &tree, iter->base.repo, &entry->tree_entry->oid)) < 0 ||
...
static int tree_iterator_advance(const git_index_entry **out, git_iterator *i)
{
	...
	while (true) {
		...
		/* we may have coalesced the contents of case-insensitively
		 * same-named directories, so do the sort now.
		 */
		if (frame->next_idx == 0 &&
		    !git_vector_is_sorted(&frame->entries))
			git_vector_sort(&frame->entries);

		/* we have more entries in the current frame, that's our next
		 * entry */
		prev_entry = tree_iterator_current_entry(frame);
		entry = frame->entries.contents[frame->next_idx];
		frame->next_idx++;

		/* we can have collisions when iterating case insensitively.
		 * (eg, 'A/a' and 'a/A').  squash this one if it's already been
		 * seen.
		 */
		...
		if ((error = tree_iterator_compute_path(
		             &iter->entry_path, entry)) < 0)
			break;

		/* if this path is before our start, advance over this entry */
		if (!iterator_has_started(
		            &iter->base, iter->entry_path.ptr, false))
			continue;
		...
		if (!iterator_pathlist_next_is(
		            &iter->base, iter->entry_path.ptr))
			continue;
		...
		if (is_tree && !iterator__include_trees(iter)) {
			/* if we've found a tree (and are not returning it to
			 * the caller) and we are autoexpanding, then we want to
			 * return the first child.  push the new directory and
			 * advance.
			 */
			if (iterator__do_autoexpand(iter)) {
				if ((error = tree_iterator_frame_push(
				             iter, entry)) < 0)
					break;
			}
		...
		/* if we are autoexpanding, then push this as a new frame, so
		 * that the next call to `advance` will dive into this
		 * directory.
		 */
...
static int
tree_iterator_advance_into(const git_index_entry **out, git_iterator *i)
{
	tree_iterator *iter = (tree_iterator *)i;
	tree_iterator_frame *frame;
	...
	/* it's legal to call advance_into when auto-expand is on.  in this
	 * case, we will have pushed a new (empty) frame on to the stack for
	 * this new directory.  since it's empty, its current_entry should be
	 * null.
	 */
...
static int tree_iterator_init(tree_iterator *iter)
{
	...
	if ((error = git_pool_init(
	             &iter->entry_pool, sizeof(tree_iterator_entry))) < 0 ||
        ...
        git_iterator_options *options)
{
	...
	static git_iterator_callbacks callbacks = {
		tree_iterator_current,      tree_iterator_advance,
		tree_iterator_advance_into, tree_iterator_advance_over,
		tree_iterator_reset,        tree_iterator_free
	};
...
{
	const filesystem_iterator_entry *a =
	        (const filesystem_iterator_entry *)_a;
	const filesystem_iterator_entry *b =
	        (const filesystem_iterator_entry *)_b;
	...
	*out = false;
	...
	if (!is_submodule && iter->base.index) {
		...
		error = git_index_snapshot_find(
		        &pos, &iter->index_snapshot, iter->base.entry_srch,
		        path, path_len, 0);
		...
		if (!error) {
			git_index_entry *e =
			        git_vector_get(&iter->index_snapshot, pos);
        ...
        filesystem_iterator_frame *new_frame)
{
	...
	if (git_ignore__lookup(
	            &new_frame->is_ignored, &iter->ignores, path,
	            GIT_DIR_FLAG_TRUE) < 0) {
		git_error_clear();
        ...
        size_t path_len)
{
	...
	*match_out = ITERATOR_PATHLIST_NONE;
	...
	if (iter->base.end_len) {
		int cmp = iter->base.strncomp(
		        path, iter->base.end, iter->base.end_len);
	...
	if (iter->base.pathlist.length) {
		...
		if (frame_entry &&
		    frame_entry->match != ITERATOR_PATHLIST_IS_PARENT)
			match = ITERATOR_PATHLIST_FULL;
		else
			match = iterator_pathlist_search(
			        &iter->base, path, path_len);
...
filesystem_iterator_is_dot_git(
        ...
        size_t path_len)
{
	/* PRSYNC: This is a PRSYNC change
	   .prsync, len = 7 */
	...
	if (git__tolower(path[len - 1]) != 'c' ||
	    ...
	    git__tolower(path[len - 5]) != 'r' ||
	    git__tolower(path[len - 6]) != 'p' || path[len - 7] != '.')
		return false;
        ...
        filesystem_iterator_entry *entry)
{
	...
	if (iter->base.type == GIT_ITERATOR_WORKDIR)
		return git_repository_hashfile(
		        &entry->id, iter->base.repo, entry->path,
		        GIT_OBJECT_BLOB, NULL);
	...
	if (!(error = git_str_joinpath(&fullpath, iter->root, entry->path)) &&
		...
		error = git_odb__hashfile(
		        &entry->id, fullpath.ptr, GIT_OBJECT_BLOB,
		        iter->oid_type);
        ...
        iterator_pathlist_search_t pathlist_match)
{
	...
	*out = NULL;
	 ...
	 */
	GIT_ERROR_CHECK_ALLOC_ADD(
	        &entry_size, sizeof(filesystem_iterator_entry), path_len);
        ...
        filesystem_iterator_entry *frame_entry)
{
	...
	if (iter->frames.size == FILESYSTEM_MAX_DEPTH) {
		git_error_set(
		        ...
		        "directory nesting too deep (%" PRIuZ ")",
		        iter->frames.size);
		return -1;
	}
	...
	while ((error = git_fs_path_diriter_next(&diriter)) == 0) {
		...
		if ((error = git_fs_path_diriter_fullpath(
		             &path, &path_len, &diriter)) < 0)
			goto done;

		path_str.ptr = (char *)path;
		path_str.size = path_len;

		if ((error = git_path_validate_str_length(
		             iter->base.repo, &path_str)) < 0)
			goto done;
		...
		/* examine start / end and the pathlist to see if this path is
		 * in it. note that since we haven't yet stat'ed the path, we
		 * cannot know whether it's a directory yet or not, so this can
		 * give us an expected type (S_IFDIR or S_IFREG) that we should
		 * examine)
		 */
		if (!filesystem_iterator_examine_path(
		            &dir_expected, &pathlist_match, iter, frame_entry,
		            path, path_len))
			continue;

		/* TODO: don't need to stat if assume unchanged for this path
		 * and we have an index, we can just copy the data out of it.
		 */

		if ((error = git_fs_path_diriter_stat(&statbuf, &diriter)) <
		    0) {
		...
		if ((error = filesystem_iterator_entry_init(
		             &entry, iter, new_frame, path, path_len, &statbuf,
		             pathlist_match)) < 0)
			goto done;
...
static int
filesystem_iterator_current(const git_index_entry **out, git_iterator *i)
{
	filesystem_iterator *iter =
	        GIT_CONTAINER_OF(i, filesystem_iterator, base);
...
static int
filesystem_iterator_advance(const git_index_entry **out, git_iterator *i)
{
	filesystem_iterator *iter =
	        GIT_CONTAINER_OF(i, filesystem_iterator, base);
	bool is_dir;
	...
	while (true) {
		...
		if ((error = filesystem_iterator_is_dir(&is_dir, iter, entry)) <
		    0)
			break;

		if (is_dir) {
			if (iterator__do_autoexpand(iter)) {
				error = filesystem_iterator_frame_push(
				        iter, entry);

				/* may get GIT_ENOTFOUND due to races or
				 * permission problems that we want to quietly
				 * swallow
				 */
				if (error == GIT_ENOTFOUND)
					continue;
...
static int
filesystem_iterator_advance_into(const git_index_entry **out, git_iterator *i)
{
	filesystem_iterator *iter =
	        GIT_CONTAINER_OF(i, filesystem_iterator, base);
	filesystem_iterator_frame *frame;
	...
	if (prev_entry) {
		...
		if ((error = filesystem_iterator_frame_push(iter, prev_entry)) <
		    0)
			return error;
	}
...
int git_iterator_current_workdir_path(git_str **out, git_iterator *i)
{
	...
	if (i->type != GIT_ITERATOR_FS && i->type != GIT_ITERATOR_WORKDIR) {
		*out = NULL;
		return 0;
	}
...
GIT_INLINE(git_dir_flag) entry_dir_flag(git_index_entry *entry)
{
	...
	return (entry && entry->mode) ?
	               (S_ISDIR(entry->mode) ? GIT_DIR_FLAG_TRUE :
	                                       GIT_DIR_FLAG_FALSE) :
...
static void filesystem_iterator_update_ignored(filesystem_iterator *iter)
{
	...
	if (git_ignore__lookup(
	            &iter->current_is_ignored, &iter->ignores, iter->entry.path,
	            dir_flag) < 0) {
		git_error_clear();
        ...
        git_iterator *i)
{
	...
	*status = GIT_ITERATOR_STATUS_NORMAL;
	 ...
	 * IGNORED.  if we find a real actual item, upgrade all the way to
	 * NORMAL and then stop.
	 *
	...
	*status = current_entry->match == ITERATOR_PATHLIST_IS_PARENT ?
	...
	while (entry && !iter->base.prefixcomp(entry->path, base)) {
		if (filesystem_iterator_current_is_ignored(iter)) {
			/* if we found an explicitly ignored item, then update
			 * from EMPTY to IGNORED
			 */
			*status = GIT_ITERATOR_STATUS_IGNORED;
			...
			/* we found a non-ignored item, treat parent as
			 * untracked */
			*status = GIT_ITERATOR_STATUS_NORMAL;
			break;
		}
...
static int filesystem_iterator_init(filesystem_iterator *iter)
{
	...
	if (iterator__honor_ignores(&iter->base) &&
	    (error = git_ignore__for_path(
	             iter->base.repo, ".gitignore", &iter->ignores)) < 0)
		return error;
        ...
        git_iterator_options *options)
{
	...
	*out = NULL;
	...
	if ((error = iterator_init_common(&iter->base, repo, index, options)) <
	    0)
		goto on_error;
	...
	iter->index = index;
	iter->dirload_flags = (iterator__ignore_case(&iter->base) ?
        ...
        git_iterator_options *given_opts)
{
	...
	return iterator_for_filesystem(
	        out, NULL, root, NULL, NULL, GIT_ITERATOR_FS, &options);
}
        ...
        git_iterator_options *given_opts)
{
	...
	return iterator_for_filesystem(
	        out, repo, repo_workdir, index, tree, GIT_ITERATOR_WORKDIR,
	        &options);
}
...
static int index_iterator_skip_pseudotree(index_iterator *iter)
{
	...
	while (true) {
		...
		if (iter->base.strncomp(
		            iter->tree_buf.ptr, next_entry->path,
...
static int index_iterator_advance(const git_index_entry **out, git_iterator *i)
{
	...
	while (true) {
		...
		/* we were not asked to expand this pseudotree.  advance over
		 * it. */
		if (iter->skip_tree) {
		...
		if (!iterator_has_started(
		            &iter->base, entry->path, is_submodule)) {
			iter->next_idx++;
			continue;
		}
		...
		/* if this is a conflict, skip it unless we're including
		 * conflicts */
		...
		/* we've found what will be our next _file_ entry.  but if we
		 * are returning trees entries, we may need to return a
		 * pseudotree entry that will contain this.  don't advance over
		 * this entry, though, we still need to return it on the next
		 * `advance`.
		 */
		if (iterator__include_trees(&iter->base) &&
		    index_iterator_create_pseudotree(
		            &entry, iter, entry->path)) {
			/* Note whether this pseudo tree should be expanded or
			 * not */
			iter->skip_tree =
			        iterator__dont_autoexpand(&iter->base);
			break;
		}
...
static int
index_iterator_advance_into(const git_index_entry **out, git_iterator *i)
{
	index_iterator *iter = GIT_CONTAINER_OF(i, index_iterator, base);
        ...
        git_iterator_options *options)
{
	...
	static git_iterator_callbacks callbacks = {
		index_iterator_current,      index_iterator_advance,
		index_iterator_advance_into, index_iterator_advance_over,
		index_iterator_reset,        index_iterator_free
	};
	...
	*out = NULL;
	...
	git_vector_set_cmp(
	        &iter->entries, iterator__ignore_case(&iter->base) ?
        ...
        void *data)
{
	...
	while (true) {
		if ((error = git_iterator_advance(&iterator_item, iterator)) <
		    0)
			goto done;
	...
	while (true) {
		...
		for (i = 0; i < cnt; i++) {
			...
			if (first_match == NULL) {
				...
				int path_diff = git_index_entry_cmp(
				        iterator_item[i], first_match);

				if (path_diff < 0) {
					/* Found an index entry that sorts
					 * before the one we're looking at.
					 * Forget that we've seen the other and
					 * look at the other iterators for this
					 * path.
					 */
					for (j = 0; j < i; j++)
						cur_items[j] = NULL;
		...
		for (i = 0; i < cnt; i++) {
			...
			error = git_iterator_advance(
			        &iterator_item[i], iterators[i]);
